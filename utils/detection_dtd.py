...
def get_ood_scores_clip(args, net, loader, test_labels, in_dist=False):
    ...
    #     data = json.load(json_file)
    with open('/media/chaod/code/clip_ood/true/dtd.json', 'r') as json_file:
        data = json.load(json_file)
    with open('/media/chaod/code/llama-main/clip_ood_datasets/specific_descirbe/full/dtd_ood_to_describe.json', 'r') as f:
                ...
                for i in range(num_rows):
                    ...
                    for i, label in enumerate(test_labels_list):
                        ...
                        if max_index_id.item() == 0 :
                            ID_value = ID_value * 0.1 # 判断最大值是否是OOD，如果是，就将OOD乘以0.8
